...
//? Custom JS
const style = require('./test/console')
const { 
...
module.exports = HandleMsg = async (bocilClient, message) => {
	...
	// Filter Banned People
        ...
        case 'speed':
        case 'ping':
            await bocilClient.sendText(from, `Pong!!!!\nSpeed: ${processTime(t, moment())} _Second_`)
            break
        case 'tnc':
        ...
        case 'help':
            ...
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? bocilClient.sendText(from, `Menu untuk Admin Grup: *${prefix}menuadmin*`) : null)
            break
        case 'menuadmin':
            if (!isGroupMsg) return bocilClient.reply(from, 'Perintah ini hanya dapat digunakan didalam grup!', id)
            if (!isGroupAdmins) return bocilClient.reply(from, 'Perintah ditolak, hanya dapat digunakan oleh admin grup!', id)
        ...
        case 'ownerbot':
            ...
            .then(() => bocilClient.sendText(from, `Jika ada masalah silahkan hubungi nomor ini ${setting.WA_Owner}`))
            break
        case 'join':
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    bocilClient.reply(from, 'Sticker mu berhasil dibuat')
                ...
                if (!isUrl(url)) { await bocilClient.reply(from, 'Maaf, link yang kamu kirim tidak silahkan kirim ulang.', id) }
                bocilClient.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? bocilClient.sendText(from, 'link trsebut yang kamu kirim tidak memuat gambar.')
                    : bocilClient.reply(from, 'Here\'s your sticker')).then(() => console.log(`Stiker sedang diproses tunggu ${processTime(t, moment())} Second`))
            } else {
                await bocilClient.reply(from, `Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker`, id)
            }
            break
        case 'stickergif':
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    bocilClient.reply(from, '[WAIT] Sedang di proses, silahkan tunggu ± 1 min!', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            bocilClient.reply(from, 'Filenya terlalu besar!', id)
                        })
                    })
                  } else {
                    bocilClient.reply(from, `[❗] Kirim gif dengan caption *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
        ...
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return bocilClient.reply(from, `Format pesan salah.\nKetik pesan ${prefix}stickergiphy *linkgiphy*`, id)
            const isGiphy = url.match(new RegExp(/https?:\/\/(www\.)?giphy.com/, 'gi'))
            const isMediaGiphy = url.match(new RegExp(/https?:\/\/media.giphy.com\/media/, 'gi'))
            if (isGiphy) {
                const getGiphyCode = url.match(new RegExp(/(\/|\-)(?:.(?!(\/|\-)))+$/, 'gi'))
                if (!getGiphyCode) { return bocilClient.reply(from, 'Gagal mengambil giphy', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                bocilClient.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    bocilClient.reply(from, 'Stickermu berhasil dibuat')
                    console.log(`Sticker akan diproses selama ${processTime(t, moment())} Second`)
                ...
                .then(() => {
                    bocilClient.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                })
                .catch(() => {
                ...
                await bocilClient.reply(from, 'Maaf, command sticker giphy hanya bisa menggunakan link dari giphy.', id)
            }
            break
        case 'meme':
        ...
        //Media
        case 'instagram':
            if (args.length == 0) return bocilClient.reply(from, `Untuk mendownload gambar atau video dari instagram\nketik: ${prefix}instagram [link_ig]`, id)
		...
		case 'tiktok':
            ...
            await bocilClient.sendFileFromUrl(from, tik, 'Sukses mengunduh Video Tiktok tanpa Watermark Menggunakan Bot WhatsApp bocilClient', '', id)
            .catch(() => {
                bocilClient.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'ytmp3':
            ...
            .then(async(res) => {
				...
				await bocilClient.sendFileFromUrl(from, `${res.thumb}`, '', `Youtube ditemukan\n\nJudul: ${res.judul}\n\nUkuran: ${res.size}\n\nAudio sedang dikirim`, id)
        ...
        case 'pantun':
            ...
            .then(body => {
                ...
                bocilClient.reply(from, randompantun.replace(/aruga-line/g,"\n"), id)
            })
            .catch(() => {
                bocilClient.reply(from, 'Ada yang Error!', id)
            })
        ...
        case 'resep':
            ...
            break
        case 'nekopoi':
            bocilClient.sendText(from, `Maaf fitur ini sedang dalam perbaikan...`)
            /* rugapoi.getLatest()
            .then((result) => {
            ...
            break
        case 'stalkig':
            if (args.length == 0) return bocilClient.reply(from, `Untuk men-stalk akun instagram seseorang\nketik ${prefix}stalkig [username]\ncontoh: ${prefix}stalkig ini.arga`, id)
        ...
        case 'cuaca':
            ...
            break
		case 'lirik':
			if (args.length == 0) return bocilClient.reply(from, `Untuk mencari lirik dari sebuah lagu\bketik: ${prefix}chord [judul_lagu]`, id)
		...
		//Fun Menu
		case 'klasmen':
			if (!isGroupMsg) return bocilClient.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
	    ...
	    case 'add':
            ...
            if (!isBotGroupAdmins) return bocilClient.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
	        if (args.length !== 1) return bocilClient.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
		            ...
		            .then(() => bocilClient.reply(from, 'Hai selamat datang', id))
                } catch {
        ...
        case 'kick':
            ...
            if (!isGroupAdmins) return bocilClient.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return bocilClient.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length === 0) return bocilClient.reply(from, 'Maaf, format pesan salah.\nSilahkan tag satu atau lebih orang yang akan dikeluarkan', id)
            if (mentionedJidList[0] === botNumber) return await bocilClient.reply(from, 'Maaf, format pesan salah.\nTidak dapat mengeluarkan akun bot sendiri', id)
            await bocilClient.sendTextWithMentions(from, `Request diterima, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await bocilClient.sendText(from, 'Gagal, kamu tidak bisa mengeluarkan admin grup.')
        ...
        case 'promote':
            ...
            if (!isGroupAdmins) return bocilClient.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return bocilClient.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return bocilClient.reply(from, 'Maaf, hanya bisa mempromote 1 user', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await bocilClient.reply(from, 'Maaf, user tersebut sudah menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await bocilClient.reply(from, 'Maaf, format pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
            await bocilClient.promoteParticipant(groupId, mentionedJidList[0])
            await bocilClient.sendTextWithMentions(from, `Request diterima, menambahkan @${mentionedJidList[0].replace('@c.us', '')} sebagai admin.`)
            break
        case 'demote':
            if (!isGroupMsg) return bocilClient.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return bocilClient.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return bocilClient.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return bocilClient.reply(from, 'Maaf, hanya bisa mendemote 1 user', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await bocilClient.reply(from, 'Maaf, user tersebut belum menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await bocilClient.reply(from, 'Maaf, format pesan salah.\nTidak dapat mendemote akun bot sendiri', id)
            await bocilClient.demoteParticipant(groupId, mentionedJidList[0])
            await bocilClient.sendTextWithMentions(from, `Request diterima, menghapus jabatan @${mentionedJidList[0].replace('@c.us', '')}.`)
            break
        case 'bye':
            if (!isGroupMsg) return bocilClient.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return bocilClient.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            bocilClient.sendText(from, 'Good bye... ( ⇀‸↼‶ )').then(() => bocilClient.leaveGroup(groupId))
            break
        case 'del':
            if (!isGroupAdmins) return bocilClient.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
		...
		case 'kasar':
			...
			if (args[0] == 'on') {
				...
				bocilClient.reply(from, 'sukses mengaktifkan fitur anti kata kasar', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				bocilClient.reply(from, 'sukses menonatifkan fitur anti kata kasar', id)
			} else {
				bocilClient.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\napasih itu? fitur apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
        ...
        if (!isOwner) return bocilClient.reply(from, `Maaf, perintah ini hanya dapat dipakai oleh ${ownerNumber}`, id)
        if (!isBotGroupAdmins) return bocilClient.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
        ...
        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return bocilClient.reply(from, `Maaf, perintah ini hanya dapat dipakai oleh ${ownerNumber}`, id)
            if (args.length == 0) return bocilClient.reply(from, `Untuk banned seseorang agar tidak bisa menggunakan commands\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
        ...
        case 'bc': //untuk broadcast atau promosi
            if (!isOwnerBot) return bocilClient.reply(from, `Maaf, perintah ini hanya dapat dipakai oleh ${ownerNumber}`, id)
            if (args.length == 0) return bocilClient.reply(from, `Untuk broadcast ke semua chat ketik:\n${prefix}bc [isi chat]`)
            let msg = body.slice(4)
            const chatz = await bocilClient.getAllChatIds()
            for (let idk of chatz) {
                var cvk = await bocilClient.getChatById(idk)
                if (!cvk.isReadOnly) bocilClient.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) bocilClient.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
            }
            bocilClient.reply(from, 'Broadcast Success!', id)
            break
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            ...
            bocilClient.reply(from, 'Success leave all group!', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            ...
            bocilClient.reply(from, 'Success clear all chat!', id)
            break
        default:
            break
        }
